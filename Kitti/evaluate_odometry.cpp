...
vector<Matrix> loadPoses(string file_name) {
  ...
  while (!feof(fp)) {
    ...
    if (fscanf_s(fp, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
...
void plotPathPlot (string dir,vector<int32_t> &roi,int32_t idx) {
  ...
  char command[1024];
  char file_name[256];
  sprintf_s(file_name,"%02d.gp",idx);
  ...
  for (int32_t i=0; i<2; i++) {
    ...
    sprintf_s(command,"cd %s; gnuplot %s",dir.c_str(),file_name);
    system(command);
  }
  
  // create pdf and crop
  sprintf_s(command,"cd %s; ps2pdf %02d.eps %02d_large.pdf",dir.c_str(),idx,idx);
  system(command);
  sprintf_s(command,"cd %s; pdfcrop %02d_large.pdf %02d.pdf",dir.c_str(),idx,idx);
  system(command);
  sprintf_s(command,"cd %s; rm %02d_large.pdf",dir.c_str(),idx);
  system(command);
}
...
void saveErrorPlots(vector<errors> &seq_err,string plot_error_dir,char* prefix) {
  ...
  // file names
  char file_name_tl[1024]; sprintf_s(file_name_tl,"%s/%s_tl.txt",plot_error_dir.c_str(),prefix);
  char file_name_rl[1024]; sprintf_s(file_name_rl,"%s/%s_rl.txt",plot_error_dir.c_str(),prefix);
  char file_name_ts[1024]; sprintf_s(file_name_ts,"%s/%s_ts.txt",plot_error_dir.c_str(),prefix);
  char file_name_rs[1024]; sprintf_s(file_name_rs,"%s/%s_rs.txt",plot_error_dir.c_str(),prefix);

  // open files
  FILE *fp_tl; fopen_s(&fp_tl, file_name_tl,"w");
  FILE *fp_rl; fopen_s(&fp_rl, file_name_rl,"w");
  FILE *fp_ts; fopen_s(&fp_ts, file_name_ts,"w");
  FILE *fp_rs; fopen_s(&fp_rs, file_name_rs,"w");
...
void plotErrorPlots (string dir,char* prefix) {
  ...
  for (int32_t i=0; i<4; i++) {
    ...
    char suffix[16];
    switch (i) {
      case 0: sprintf_s(suffix,"tl"); break;
      case 1: sprintf_s(suffix,"rl"); break;
      case 2: sprintf_s(suffix,"ts"); break;
      case 3: sprintf_s(suffix,"rs"); break;
    }
       
    // gnuplot file name
    char file_name[1024]; char full_name[1024];
    sprintf_s(file_name,"%s_%s.gp",prefix,suffix);
    sprintf_s(full_name,"%s/%s",dir.c_str(),file_name);
    ...
    // create pdf and crop
    sprintf_s(command,"cd %s; ps2pdf %s_%s.eps %s_%s_large.pdf",dir.c_str(),prefix,suffix,prefix,suffix);
    system(command);
    sprintf_s(command,"cd %s; pdfcrop %s_%s_large.pdf %s_%s.pdf",dir.c_str(),prefix,suffix,prefix,suffix);
    system(command);
    sprintf_s(command,"cd %s; rm %s_%s_large.pdf",dir.c_str(),prefix,suffix);
    system(command);
  }
}
...
void saveStats (vector<errors> err,string dir) {
  ...
  // open file  
  FILE* fp;
  fopen_s(&fp, (dir + "/stats.txt").c_str(), "w");
...
int32_t evaluate (int32_t argc,char *argv[]) {
